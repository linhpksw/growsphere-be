...
export const allProduct = async (req: Request, res: Response) => {
    try {
        ...
        const products = await Product.find({}).sort({ date: -1 }).skip(skip).limit(parsedLimit);
...
export const getSinglePorduct = async (req: Request, res: Response) => {
    try {
        ...
        // 1) Validate that "id" is a valid ObjectId
        if (!Types.ObjectId.isValid(id)) {
            return res.status(400).send({ message: 'Invalid product ID' });
        }
        const objectId = new Types.ObjectId(id);

        // 2) Find the product by its ObjectId
        const product = await Product.findById(objectId).lean();
        if (!product) {
            return res
                .status(404)
                .send({ data: null, rettingsData: { rettings: 0, averageRating: 0 } });
        }

        // 3) Aggregate reviews, matching productId as an ObjectId
        const rettingsAgg = await Reviews.aggregate([
        ...
        const avgRating = rettingsAgg.length > 0 ? rettingsAgg[0].averageRating : 0;
        const totalRettings = rettingsAgg.length > 0 ? rettingsAgg[0].totalCount : 0;
        ...
        console.error('Error in getSingleProduct:', e);
...
export const deleteSingleImg = async (req: Request, res: Response) => {
    try {
        ...
        await Product.updateOne({ _id: id }, { $pull: { productImages: deletetImg } });
...
export const getFiltredPorduct = async (req: Request, res: Response) => {
    try {
        ...
        const products = await Product.find(query).sort({ date: -1 }).skip(skip).limit(parsedLimit);
        const totalProductsCount = await Product.countDocuments();
        const totalPages = Math.ceil(totalProductsCount / parsedLimit);
        res.status(200).send({
            products,
...
export const bestSellingProduct = async (req: Request, res: Response) => {
    try {
        ...
        const formattedProducts: formattedProductsType[] = bestSoldProducts.map((product) => {
        ...
        const orderedProducts = formattedProducts.map((formattedProduct) =>
            products.find((product) => product._id.toString() === formattedProduct.productIds)
        );
...
export const Review = async (req: Request, res: Response) => {
    try {
        ...
        reviews.forEach((review) => {
            ...
            if (!uniqueEmailReviews[email] || review.retting > uniqueEmailReviews[email].retting) {
...
export const allOfferProduct = async (req: Request, res: Response) => {
    try {
        ...
        const parsedLimit = parseInt(limit as string);
        const skip = (parsedPage - 1) * parsedLimit;
        const products = await OffProduct.find({}).sort({ date: -1 }).skip(skip).limit(parsedLimit);
