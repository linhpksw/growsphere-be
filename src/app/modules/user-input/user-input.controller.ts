...
export const getReview = async (req: Request, res: Response) => {
    try {
        ...
        // 1) Validate that "id" is provided and is a valid ObjectId
        if (typeof id !== 'string' || !Types.ObjectId.isValid(id)) {
            return res.status(400).send({ message: 'Invalid or missing product ID' });
        }
        const productObjectId = new Types.ObjectId(id);

        // 2) Find all reviews whose productId matches the ObjectId, sorted by date descending
        ...
        return res.status(500).send({ message: 'Custom error' });
    }
};
    ...
    try {
        ...
        if (result.matchedCount === 1) {
            ...
            await Product.updateOne(
                ...
                {
                    arrayFilters: [{ element: oldRatting }],
                }
            );
...
export const getRefundInfo = async (req: Request, res: Response) => {
    try {
        ...
        const products = await RefundInfo.find({}).sort({ date: -1 }).skip(skip).limit(parsedLimit);
